...
def main():
    ...
    # ───────────────────────────────────────────
    reflow_transform = T.Compose([
        T.Resize((config.img_size, config.img_size)),
        T.Grayscale(num_output_channels=config.model.channels),
        T.RandomHorizontalFlip(),
        T.ToTensor(),
        T.Lambda(lambda t: (t * 2) - 1)
    ])
    ds = ImageFolder(root=config.reflow_dir, transform=reflow_transform)
    dl = DataLoader(ds, batch_size=config.batch_size, shuffle=True, num_workers=16)
    # ───────────────────────────────────────────
